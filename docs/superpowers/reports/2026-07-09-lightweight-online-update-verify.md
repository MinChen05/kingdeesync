...
- `python -m pytest tests\test_update_service.py tests\test_updater_dry_run.py tests\test_release_package.py -q`：通过，37 passed。
- `python -m pytest tests\test_gui_windows11_shell.py -q -k "settings_page or launch_updater"`：通过，19 passed，269 deselected，1 个既有 `pythonjsonlogger` deprecation warning。
...
- updater 在删除旧文件前校验更新包内存在主程序文件，缺失时拒绝安装并保留旧目录内容。（原因：避免坏包导致旧客户端不可用）
...
- 主程序入口支持 `updater` 子命令，打包后可用同一个 exe 启动独立 updater 进程。（原因：PyInstaller 发布包需要可执行的 updater 入口）
