...
- [x] 1.1 在 `src/config/form-queries.json` 新增“采购入库单”查询配置，初始 FormId 使用 `STK_InStock`，字段先覆盖基础明细并保留 dry-run 校准记录。
- [x] 1.2 在 `src/config/tables.json` 新增“采购入库单”目标表和 writer 映射。
- [x] 1.3 在 `config.example.ini` 的 staging 配置中加入采购入库单目标表。
- [x] 1.4 扩展配置加载或配置校验测试，确认“采购入库单”可被发现。

## 2. Writer 与数据准备

- [x] 2.1 新增 `insert_purchase_instock` writer，并注册到 `src/core/writers_registry.py`。
- [x] 2.2 新增 `_prepare_purchase_instock_data` 数据准备方法，覆盖基础明细字段转换。
- [x] 2.3 为缺少 `FID`、分录主键或单号的记录增加跳过逻辑和警告日志。
- [x] 2.4 在 SQL Server upsert/staging 主键映射中补充采购入库单分录级幂等键。

## 3. SQL Server 表结构与索引

- [x] 3.1 定义采购入库单非报表业务目标表字段，保留数据类型与现有写入引擎兼容。
- [x] 3.2 如目标表不存在，新增建表或自动补列路径；如涉及主键、唯一键或字段顺序调整，先保留数据并校验行数。
- [x] 3.3 为分录主键和修改时间新增必要索引。
- [x] 3.4 明确本次不修改报表相关表结构。

## 4. 测试与 Dry-run

- [x] 4.1 新增 writer registry 测试，确认 `insert_purchase_instock` 已注册。
- [x] 4.2 新增采购入库单字段准备测试，覆盖正常记录、空分录主键、空单号和大小写字段兼容。
- [x] 4.3 新增 upsert/staging 测试，确认采购入库单使用分录级幂等键。
- [x] 4.4 新增或更新 dry-run 脚本说明，输出金蝶响应样例、字段校准结果、预计写入量和跳过量。
