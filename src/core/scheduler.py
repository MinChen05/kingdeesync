...
class AutoSyncScheduler:
    ...
    def __init__(self):
        ...
        self._previous_exec_time: datetime | None = None  # 上上次执行时间（用于检测间隔异常）
    ...
    def configure_sync(self, forms: List[str], sync_type: SyncType = SyncType.INCREMENTAL,
        ...
        if sync_type != SyncType.INCREMENTAL:
            logger.info("定时调度仅支持增量同步，忽略同步类型: %s", sync_type.value)
        self.sync_type = SyncType.INCREMENTAL
        ...
        config_manager.update_config('SYNC', 'sync_type', self.sync_type.value)

        forms_log = "同步所有表单" if not self.sync_forms else str(self.sync_forms)
        logger.info(f"已配置定时同步: 表单={forms_log}, 类型={self.sync_type.value}, 间隔={interval_minutes}分钟")
    ...
    def _execute_sync(self):
        ...
        effective_sync_type = SyncType.INCREMENTAL
        if self.sync_type != effective_sync_type:
            logger.info("定时调度仅支持增量同步，已从 %s 切换为 incremental", self.sync_type.value)
            self.sync_type = effective_sync_type
        mode_text = "增量同步 (推荐)"
    ...
    def _get_configured_interval_seconds(self) -> int:
        """从 schedule 库中提取配置的同步间隔（秒）"""
        import schedule as schedule_lib

        try:
            for job in schedule_lib.jobs:
                if (
                    hasattr(job, "job_func")
                    and job.job_func is not None
                    and getattr(job.job_func, "__self__", None) is self
                    and hasattr(job, "period")
                ):
                    return int(job.period) * 60
        except Exception:
            pass
        return 3600
    ...
    def update_sync_type(self, sync_type: SyncType):
        """更新同步类型"""
        if sync_type != SyncType.INCREMENTAL:
            logger.info("定时调度仅支持增量同步，忽略同步类型更新请求: %s", sync_type.value)
        self.sync_type = SyncType.INCREMENTAL
        config_manager.update_config('SYNC', 'sync_type', self.sync_type.value)
        logger.info(f"同步类型已更新为 {self.sync_type.value}")
    ...
    def load_config_and_start(self):
        ...
        try:
            ...
            # 设置同步类型（定时调度固定为增量）
            self.sync_type = SyncType.INCREMENTAL
            if sync_config.get('sync_type', SyncType.INCREMENTAL.value) != SyncType.INCREMENTAL.value:
                config_manager.update_config('SYNC', 'sync_type', SyncType.INCREMENTAL.value)
